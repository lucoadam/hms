<template>
    ...
    <v-form v-model="valid" ref="zone_form">
        <v-layout>
            <v-flex md5>
                <h3>Zone Data</h3>
...
<script>
    import ZoneApi from '../../../../api/Zone'
        ...
        name: 'zone-form',
        created () {
            // ZoneApi.role().then((res)=>{
            //     this.roles = res.body
            // })
            // ZoneApi.findAllPermissions().then((res) => {
        ...
        methods: {
            clear () {
                this.$refs.zone_form.reset()
            },
            submit () {
                ZoneApi.save(this.zone).then((response) => {
                    this.$router.push({ name: 'admin.zones.index' })
                })
            }
        }
    }
